...
public class Solution {
    public long subarraySum(int[] A) {
        int N = A.length;
        long sum=0,left,right,contribution;
		for(int i=0;i<N;i++){//starting index
			left = (i+1);
            right = (N-i);
            contribution = left*right;
            sum+= (long) A[i] *contribution;
		}
        return sum;
    }
}
...
Hint 1
This is a straightforward problem for solving in O(N2). 
How can we improve it to O(N)?
Think how each element is contributing to the sum.
